	...
	public Set<String> getLanguages() {
		if (isAvailable()) {
			try {
				...
				String[] langfiles = new File(WellKnowns.OCR_FOLDER.getPath(),
						"tessdata").list(new FilenameFilter() {
					public boolean accept(File dir, String name) {
						return name.endsWith(".traineddata");
					}
				});
				String[] langcodes = new String[langfiles.length];
				for (int i = 0; i < langfiles.length; i++) {
					langcodes[i] = langfiles[i].replace(".traineddata", "");
				}

				return new TreeSet<>(Arrays.asList(langcodes));
			} catch (Throwable ex) {
